...
//            emit sig_loading_contact_user();
            ...
            _load_pending = true;// 标记正在加载

            QTimer::singleShot(100, [this](){
                _load_pending = false;// 100ms 后重置加载标记,可以触发加载更多联系人
...
void ContactUserList::addContactUserList()
{
    ...
    //第二行是:新的朋友ConUserItem
    //向QListWidget里添加我们自定义的Item的逻辑
    //1.先创立我们要添加的_add_friend_item
    ...
    //2.再创建QListWidgetItem *add_item;
    QListWidgetItem *add_item = new QListWidgetItem;
    //qDebug()<<"chat_user_wid sizeHint is " << chat_user_wid->sizeHint();
    add_item->setSizeHint(_add_friend_item->sizeHint());

    //3.把第二步创建的*add_item加到ContactUserList里
    this->addItem(add_item);
    //4.最后this->setItemWidget(add_item, _add_friend_item);用我们的_add_friend_item替换第三部添加的add_item
...
void ContactUserList::slot_item_clicked(QListWidgetItem *item)
{
    ...
    if(itemType == ListItemType::CONTACT_USER_ITEM){
       // 创建对话框，提示用户
       qDebug()<< "点击了联系人item--跳转一下:contact user item clicked ";
