...
public:
    ...
    //更新联系人数量、加载信息
    std::vector<std::shared_ptr<FriendInfo>> GetChatListPerPage();//（右边）获取一页聊天条目（需要判断有没有加载完成）
    bool IsLoadChatFin();//聊天条目是否加载完
    void UpdateChatLoadedCount();//更新聊天条目
    std::vector<std::shared_ptr<FriendInfo>> GetConListPerPage();//（左边）获取一页联系人（需要判断有没有加载完成）
