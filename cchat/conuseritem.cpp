...
void ConUserItem::SetInfo(std::shared_ptr<AuthInfo> auth_info)//去看UserInfo的构造，可以接受一个<AuthInfo>类型的智能指针，把智能指针执行的内容拷贝下来给UserInfo初始化
{
    ...
    qDebug() << "ConUserItem[auth_info]查看图片路径是否正确：icon path:" <<_info->_icon;//---
    //加载图片
...
void ConUserItem::SetInfo(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    qDebug() << "ConUserItem[auth_rsp]查看图片路径是否正确：icon path:" <<_info->_icon;//---
    //加载图片
    QPixmap pixmap(_info->_icon);
    qDebug() << "pixmap isNull:" << pixmap.isNull();//---
    //图片等比例缩放;图片里的内容允许缩放
...
void ConUserItem::SetInfo(int uid, QString name, QString icon)
{
    ...
    //加载图片
    qDebug() << "ConUserItem[uid,name,icon]查看图片路径是否正确：icon path:" <<_info->_icon;//---
    //加载图片
    QPixmap pixmap(_info->_icon);
    qDebug() << "pixmap isNull:" << pixmap.isNull();//---
    //图片等比例缩放;图片里的内容允许缩放
