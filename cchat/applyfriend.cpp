...
ApplyFriend::ApplyFriend(QWidget *parent) : QDialog(), ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("果粒橙C"));
...
void ApplyFriend::SlotLabelEnter()
{
    ...
    auto* lb = new ClickedLabel(ui->lb_list);//创建标签
    lb->SetState("normal", "hover", "pressed", "selected_normal",
        "selected_hover", "selected_pressed");//设置状态
    lb->setObjectName("tipslb");
    lb->setText(text);
    //可以点击-->槽函数动作
    connect(lb, &ClickedLabel::clicked, this, &ApplyFriend::SlotChangeFriendLabelByTip);
...
void ApplyFriend::SlotApplySure()
{
    ...
    //组织出json数据发送请求
    QJsonObject jsonObj;
    auto uid = UserMgr::GetInstance()->GetUid();//获取自己的uid发给对方

    jsonObj["uid"] = uid;
    auto name = ui->name_ed->text();//自己的名字发给对方
    if(name.isEmpty()){
        name = ui->name_ed->placeholderText();//默认的名字
    }
    jsonObj["applyname"] = name;//申请名字=自己的名字

    //给对方的备注
    auto bakname = ui->back_ed->text();
    if(bakname.isEmpty()){
        bakname = ui->back_ed->placeholderText();//默认的
    }

    jsonObj["bakname"] = bakname;
    jsonObj["touid"] = _si->_uid;//发给谁，对方的uid，通过si取出来

    //将json对象放到文件中，便于序列化发送数据
    QJsonDocument doc(jsonObj);
    QByteArray jsonData = doc.toJson(QJsonDocument::Compact);//压缩

    //发送tcp请求给chat server
    qDebug() << "即将发送sig_send_data信号";
