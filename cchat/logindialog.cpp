    ...
    ui(new Ui::LoginDialog)
{
    ...
    //由reg_btn按钮点击会发出PushButton的clicked信号，由LoginDialog接收，马上会发送自己定义的信号。-> 信号发给mainWindow。收到后会调用自己写的槽函数执行切换界面操作。
...
{
    qDebug()<<"desturt LoginDlg";
    delete ui;
}

void LoginDialog::initHttpHeadlers()
{
    //注册获取登录回包逻辑
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj){
        ...
        //发送信号通知tcpMgr发送长链接
        //这是服务器返回来的信息（uid/host/port/token）
        ServerInfo si;
...
{
    // 加载图片
        QPixmap originalPixmap(":/res/rc(1).png");
          // 设置图片自动缩放
...
void LoginDialog::slot_login_mod_finish(ReqId id, QString res, ErrorCodes err)
{
    ...
    //调用对应的逻辑,根据id回调。解析收到的内容，发给相应的服务器
