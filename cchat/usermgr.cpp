...
std::shared_ptr<FriendInfo> UserMgr::GetFriendById(int uid)
{
    ...
    //*find_it 的类型是 FriendInfo&，函数返回类型是 FriendInfo，编译器会做一次拷贝（或移动）,还可能会悬空引用。
    //可以直接用find_it->value()
}
...
std::vector<std::shared_ptr<FriendInfo>> UserMgr::GetChatListPerPage() {
    ...
    int end = begin + CHAT_COUNT_PER_PAGE;//一页加载的数量（global.h中定义了13个）

    //起始位置大于整个好友列表数量
    if (begin >= _friend_list.size()) {
        return friend_list;
    }
    //结束位置大于整个好友列表，更新结束位置到好友列表位置
