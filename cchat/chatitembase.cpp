...
ChatItemBase::ChatItemBase(ChatRole role, QWidget *parent) : QWidget(parent), m_role(role)
{
    ...
    //基础的控件的间距
    pGLayout->setVerticalSpacing(5);//垂直间距
    pGLayout->setHorizontalSpacing(3);//水平间距
    pGLayout->setMargin(3);//边界
    ...
    if(m_role == ChatRole::Self)
    {
        ...
        pGLayout->addWidget(m_pIconLabel, 0, 2, 2,1, Qt::AlignTop);//头像放的位置，第0行第1列，占2行一列，头像放的靠顶部
        pGLayout->addItem(pSpacer, 1, 0, 1, 1);
        pGLayout->addWidget(m_pBubble, 1 , 1, 1,1);
        pGLayout->setColumnStretch(0, 2);//两行的拉伸，拉伸比例：第0列占2/5即40%，第一列60%
