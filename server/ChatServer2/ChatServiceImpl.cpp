...
// gRPC 服务的实现部分，用于处理其他聊天服务器通过 gRPC 发送过来的请求。主要功能是接收跨服务器请求，然后在本服务器上查找目标用户并推送通知。
ChatServiceImpl::ChatServiceImpl()
{

}

//1. **好友申请通知 (`NotifyAddFriend`)**  
//  - 接收其他服务器发来的好友申请请求
//  - 检查目标用户是否在线（在本服务器）
//  - 如果在线，将申请信息通过 TCP 推送给目标用户
...
//2. * *好友验证通知(`NotifyAuthFriend`) * *
//	- 接收其他服务器发来的好友验证结果
//	- 检查目标用户是否在线
//	- 如果在线，将验证结果通过 TCP 推送给目标用户
...
//3. * *文本消息转发(`NotifyTextChatMsg`) * *
//	-接收其他服务器转发的聊天消息
//	- 检查目标用户是否在线
//	- 如果在线，将消息通过 TCP 推送给目标用户
...
//4. * *用户信息获取(`GetBaseInfo`) * *
//	-从 Redis 或 MySQL 获取用户信息（与之前相同）
