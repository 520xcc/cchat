...
//GetBaseInfo函数的功能是从Redis缓存或MySQL数据库中获取用户的基本信息
bool LogicSystem::GetBaseInfo(std::string base_key, int uid, std::shared_ptr<UserInfo>& userinfo)
{
	...
	std::cout << "[GetBaseInfo] Redis result: " << info_str << std::endl;//---
	...
	if (b_base) {
		...
		if (user_info) {
			std::cout << "[GetBaseInfo] MySQL uid=" << uid
				<< " name=" << user_info->name
				<< " icon=" << user_info->icon << std::endl;
		}
		else {
			std::cout << "[GetBaseInfo] MySQL 没查到 uid=" << uid << std::endl;
		}
		...
		std::cout << "[GetBaseInfo] write Redis icon=" << userinfo->icon << std::endl; //---
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	//去Redis中查询用户信息：Redis里查不到就去MySql里查，然后更新到Redis里。
	//在GetBaseInfo函数里。base_key, uid, user_info第一个参数去Redis里查要用的，第二个去MySql里查要用的。
...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	...
	std::cout << "收包json解析没问题：user login uid is :" << uid << "applyname is :" << applyname << "backname is :" << backname
	...
	Defer defer([this, &rtvalue, session]() {
		...
		std::cout << "无论成功与否都要发 ID_NOTIFY_ADD_FRIEND_REQ 给客户端：" << std::endl;
	...
	//查到了：
	// 一、和from_uid同一个server，查找有没有session查到直接session转发给客户端。
	// 二、不是同一个server，当前ChatServer--Grpc--ChatServer（目的）--client2
	...
	if (to_ip_value == self_name) {
		...
		if (session) {  //在内存里有uid---session的绑定关系直接发，不然就去Redis查
			...
			std::cout << "ID_NOTIFY_ADD_FRIEND_REQ 发送给客户端：" << std::endl;
	...
	//二、不是同一个ChatServer
	// 构造rpc请求，把信息包含进去
	std::cout << "不同的服务器要发rpc请求ID_NOTIFY_ADD_FRIEND_REQ" << std::endl;//---
