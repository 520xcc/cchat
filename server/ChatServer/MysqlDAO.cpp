...
MysqlDAO::MysqlDAO()
{
	...
	//配置放入池子
	pool_.reset(new MySqlPool(host + ":" + port, user, pwd, schema, 5));//创建了新的连接池，交给智能指针管理
}

MysqlDAO::~MysqlDAO() {
	pool_->Close();//掀掉pool的析构，再调mysql的析构
}
//注册逻辑
int MysqlDAO::RegUser(const std::string& name, const std::string& email, const std::string& pwd)
{
	...
	try {
		...
		//后面语句不用带参数，不需要prepare。取出之前准备的result值，放到res中
...
bool MysqlDAO::CheckPwd(const std::string& email, const std::string& pwd, UserInfo& userInfo) {
	...
	Defer defer([this, &con]() {
		pool_->returnConnection(std::move(con));//返还连接池子里面的连接给其他用户
		});
	try {
		//准备Sql语句
		...
		//执行查询
		std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());//取出email对应的密码
...
std::shared_ptr<UserInfo> MysqlDAO::GetUser(std::string name)
{
	...
	try {
		...
		//检查错误原因
		std::cout << "Searching user with name=[" << name << "], length=" << name.size() << std::endl;
...
bool MysqlDAO::GetFriendList(int self_id, std::vector<std::shared_ptr<UserInfo> >& user_info_list) {
	...
	try {
		// 准备SQL语句, 根据自己的id和查friend表找到所有好友，限制条数返回列表
